...
public class UsuarioDao {
	...
	try {
	    ...
	    String sql = "INSERT INTO usuario (nome, email, login, senha, habilitado) VALUES (?,?,?,?,?)";
	    PreparedStatement stmt = connection.prepareStatement(sql);
	    stmt.setString(1, usuario.getNome());
	    stmt.setString(2, usuario.getEmail());
	    stmt.setString(3, usuario.getLogin());
	    stmt.setString(4, usuario.getSenha());
	    stmt.setBoolean(5, usuario.isHabilitado());
	    stmt.execute();
	    stmt.close();
	    connection.close();
	...
	try {
	    ...
	    PreparedStatement stmt = this.connection.prepareStatement("SELECT * FROM usuario WHERE habilitado = true ORDER BY nome");
